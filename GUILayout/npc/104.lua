...
local function getCostState()
    ...
    local itemName = cost[1] and cost[1][1] or ""
    local need = toNumber(cost[1] and cost[1][2], 0)
    ...
    return {
        name = itemName,
        need = need,
        have = getItemCountByName(itemName),
        itemData = itemData,
    }
end
...
local function buildDescFromAttrs(name, attrs)
    ...
    if #attrIds == 2 and attrIds[1] == 200 and attrIds[2] == 201 then
        return string.format("攻击速度+%d%%", values[1] or 0)
    end
...
local function getChoiceState(choice)
    ...
    local name = getDisplayName(choice.name)
    local desc = tostring(choice.desc or "")
    if desc == "" then
...
local function buildChoiceRichText(choice, idx, selected)
    local state = getChoiceState(choice)
...
local function renderCost(node, data)
    ...
    local iconBg = GUI:Image_Create(node, "cost_item_bg", 314, 44, "res/wy/public/58_58_kuang.png")
    ...
    createRich(node, "cost_count", 252 + 85, 40, 120, 17,
        string.format("<font color='%s'>%d</font><font color='#f7f7de'>/%d</font>", countColor, costState.have, costState.need),
        "#f7f7de", 0, 0.5)
