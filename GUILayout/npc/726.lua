...
local SPEC = {
    folder = "幽影",
    background = "面板-.png",
    reward = {x = 274 - 59 + 50, y = 205 - 25, scale = 0.9},
    cost = false,
    progressHidden = true,
    images = {
        {name = "dialog", skin = "对话.png", x = 337, y = 366 + 40},
        {name = "req_title", skin = "任务要求.png", x = 208 + 10, y = 282 + 40},
        {name = "req_content", skin = "要求内容.png", x = 424 + 20, y = 282 + 7},
        {name = "reward_title", skin = "任务奖励.png", x = 208, y = 222 - 20},
    },
    buttons = {
        [1] = {x = 337, y = 150 - 40, w = 294, h = 50, skin = "“我完成了！”.png"},
    },
}
...
local function _resolve_bg()
    local customBg = _asset(SPEC.background)
    if customBg then
        return customBg, false
    end
    ...
    for idx, action in ipairs(list) do
        ...
        if not actionSpec.hidden then
            ...
            elseif (useDemo and actionSpec.visible ~= true) or actionSpec.transparent then
                ...
                if state <= 0 and action.ew == 1 then
                    skin = _skin("res/custom/all_story_mission/2/btn_take.png") or skin
                end
