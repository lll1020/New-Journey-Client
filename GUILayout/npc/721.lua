...
local function _render_dialog_item_group(node, cfg, dialog)
    if not dialog then
        return
    end
    ...
    if dialog.give or dialog.rewardTitle or dialog.rewardPos then
        local rewardItems = dialog.give or _pick_items(cfg, "jl")
        _render_dialog_item_list(node, "dialog_reward", rewardItems, dialog.rewardTitle, dialog.rewardTitlePos, dialog.rewardPos, false)
    end
end

local function _render_dialogs(node, key, data, cfg)
    if type(SPEC.dialogs) ~= "table" then
        return false
    end
    local story = data.T_dljq or {}
    local state = _toint(story[key])
    if state >= 2 or _toint(story[key .. "_got"]) >= 1 then
        npc.dialogStep = 3
    else
        npc.dialogStep = math.max(1, math.min(3, _toint(npc.dialogStep, 1)))
    end
    ...
    _render_dialog_item_group(node, cfg, dialog)
    ...
    if btnSkin == DEFAULT_BUTTON then
        GUI:Button_setTitleText(btn, step < 3 and "继续" or "提交任务")
    ...
    GUI:addOnClickEvent(btn, function()
        ...
        if step == 2 then
            SL:SendLuaNetMsg(100, NPC_ID, 2, 0, "")
            return
        end
...
_render = function()
    ...
    _render_images(node)
    if _render_dialogs(node, key, npc.data, curCfg) then
        return
    end
...
function npc.main(_npcid, p2, _p3, msgData)
    if p2 == 0 then
        ...
        npc.data.sg_data = npc.data.sg_data or {}
        local key = "npc_" .. tostring(NPC_ID)
        if _toint(npc.data.T_dljq[key]) >= 2 or _toint(npc.data.T_dljq[key .. "_got"]) >= 1 then
            npc.dialogStep = 3
        else
            npc.dialogStep = 1
        end
