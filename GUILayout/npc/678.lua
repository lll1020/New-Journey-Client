...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function xjm_678()
        npc.xjm_window = NPC_UI_HELPER.ensureWindow(nil, 30, {
            ...
            closeButton = {x = 768, y = 432},
        })
        local bg = npc.xjm_window.bg
        local xjm_node = npc.xjm_window.node
        npc.idx = npc.idx or 1

        local function renderRewardDesc()
            GUI:removeChildByName(xjm_node, "reward_title")
            GUI:removeChildByName(xjm_node, "reward_desc")
    ...
    local function UI_updata(node)
        ...
        local ch_kuang = GUI:Image_Create(node, "ch_kuang", 413, 110, "res/wy/public/70_70_k.png")
        ...
        elseif npc.data.T_dljq[key] == 1 then
            local count = 0
            for i = 1, 8 do
                if npc.data.T_dljq[key.."_"..i] then
                    count = count + 1
                end
            end
            if count >= 8 then
