...
local state_info = {
    ...
    [2] = {color = "#F4D179", text = "可领取"},
    [3] = {color = "#54FF9F", text = "已通关"},
}
...
local function getTrialState(data, idx)
    ...
    local value = tonumber(npc68[tostring(idx)] or -1) or -1
    if value >= 2 then
        return 3
    end
    if value == 1 then
...
local function isAxeNeed(itemName)
    itemName = tostring(itemName or "")
    return itemName == "切割之斧[lv10]" or itemName == "切割之斧LV10"
end

local function hasNeedAxe()
    local item = SL:GetMetaValue("EQUIP_DATA", 9)
    ...
    return itemName ~= "" and string.find(itemName, "切割之斧", 1, true) ~= nil
end
...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function createLabel(labelNode, idx)
        ...
        local model = GUI:Effect_Create(labelNode, "monster_model", 150, 200, 2, config.mob_shape or 0, 0, 0, 5)
        GUI:setScale(model, config.scale or 1)

        createText(labelNode, "mob_name", 150 + 25, 350, 24, "#EDE4C6", config.mob_name or "未知守护兽", 0.5, 0.5)
        ...
        createRich(labelNode, "content_rich", 382 - 32 + 30, 230 + 15, 250, 16,
            "1. 进入专属试炼副本，\n     限时击败守护兽\n"
            .. "2. 通关后获得\n     本命灵根常驻特效\n")

        createText(labelNode, "enter_req", 175, 118 + 23 - 5, 25, "#F03022", "仅限当前本命灵根", 0, 0.5)
        createText(labelNode, "reward_text", 175, 80 + 22 - 5, 25, "#BEFF26", "本命灵根常驻特效", 0, 0.5)

        -- createText(labelNode, "need_item_title", 600 - 60, 94, 18, "#FFD36A", "建议拥有", 0.5, 0.5)
        ...
        local trialCost = npc._config.cost or {}
        if type(trialCost) == "table" and #trialCost > 0 then
            createText(labelNode, "trial_cost_title", 382, 124, 20, "#F6E7C2", "挑战消耗：", 0, 0.5)
            local costNode = checkItemNumByTable_img_kuang(trialCost, nil, GUI:Node_Create(labelNode, "trial_cost", 0, 0))
            GUI:setPosition(costNode, 480 - 30, 88)
        end
        ...
        elseif state == 2 then
            local activeBtn = GUI:Button_Create(labelNode, "active_btn", 150 + 37, -8 + 46, "res/wy/public/an_lqjl.png")
            ...
            GUI:addOnClickEvent(enterBtn, function()
                ...
                if isAxeNeed(needItem) then
                    if not hasNeedAxe() then
                        SL:ShowSystemTips("<font color='#FFCC66'>进入金灵根试炼需要先穿戴切割之斧[lv10]或更高等级装备</font>")
                        return
                    end
                elseif needItem ~= "" and not hasNeedTitle(needItem) then
    ...
    elseif p2 == 2 then
        ...
        npc.data.T_dljq = npc.data.T_dljq or {}
        ensureTable(npc.data.T_dljq, "npc_68")[tostring(p3)] = 2
        npc.titles_sign = tonumber(p3) or npc.titles_sign or 1
        updateUI(npc.node)
    end
end
