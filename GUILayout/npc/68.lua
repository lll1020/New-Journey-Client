...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function GUI_createLabel(Label_node,idx)
        ...
        local state = (npc.data.T_dljq["npc_68"][""..idx] and npc.data.T_dljq["npc_68"][""..idx] == 1) and 2 or 1
        ...
        -- , "fonts/500.ttf")
        GUI:Text_Create(Label_node, "yq",180,-10 + 95 + 38, 20, "#F03022", config.yq)
        GUI:Text_Create(Label_node, "jl",180,-10 + 95, 20, "#BEFF26", config.jl)

        local kuang = GUI:Image_Create(Label_node, "kuang2", 513, 10, "res/wy/public/58-60.png")
        -- UiTools.showItemData(kuang, SL:GetMetaValue("ITEM_DATA",SL:GetMetaValue("ITEM_INDEX_BY_NAME",config.itme.."[称号]")))

        if state == 1 then
            local Button= GUI:Button_Create(Label_node, "Button",150,-10, "res/custom/four_city/lgsl/btn.png")
            GUI:addOnClickEvent(Button, function()
                SL:SendLuaNetMsg(100, npcid, 1, idx, "")
            end)
            GUI:Image_Create(Label_node, "ywc",250,0, "res/custom/four_city/lgsl/7_1.png")
        else
            GUI:Image_Create(Label_node, "ywc",250,0, "res/custom/four_city/lgsl/7_1.png")
        end
    end
    ...
    local function UI_updata(node) --界面渲染
        ...
        npc.titles_sign = 1
        for i = 1, 5 do
            local cbl_item = GUI:Button_Create(npc.cbl_list, "item" .. i, 0, 0, "res/custom/four_city/lgsl/list/"..(npc.titles_sign == i and "l" or "n").."/"..i..".png")
