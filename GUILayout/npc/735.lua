...
local SPEC = {
    folder = "洛水杜康",
    reward = {x = 178 + 240, y = 88 + 20 + 5 + 10},
    cost = {x = 260 + 250 + 121 - 23, y = 202 + 5 + 10},
    buttons = {
        [1] = {x = 502 + 140, y = 130, skin = "res/custom/all_story_mission/5/717/2.png"},
        [2] = {hidden = true},
    },
}
...
local function _render_item_group(node, cfg, group)
    ...
    if group.kind == "cost" then
        widget = checkItemNumByTable_img_kuang_compact(items, nil, root, 68)
    else
...
local function _render_buttons(node, key, data, cfg, useDemo)
    ...
    for idx, action in ipairs(list) do
        ...
        if not actionSpec.hidden then
            ...
            if state >= 2 and action.ew == 1 and SPEC.showComplete == true then
