...
local function openPrerequisiteGuide(preNpcid)
    SL:OpenCommonTipsPop({
        str = "当前条件未满足，请先将斗笠提升到【斗笠[lv10]】，是否立即前往提升？",
        btnType = 2,
        callback = function(atype)
            if atype == 1 then
                SL:SendLuaNetMsg(105, tonumber(preNpcid) or 8, tonumber(preNpcid) or 8, 0, "")
            end
        end,
    })
end
...
local WINDOW_OPTS = {
    background = {skin = "res/custom/two_city/41_bg.png", eff = true},
    title = {x = 56, y = 464, skin = "res/custom/two_city/41_title.png"},
}
...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function UI_updata(node) --界面渲染
        ...
        GUI:Image_Create(node, "x", 0, 0.00, "res/custom/two_city/41_x.png")
        ...
        if item then
            ...
            for i, v in ipairs(npc._config.cost) do
                GUI:ItemShow_Create(node, "cost"..(i > 2 and i+1 or i), 382 + ((i > 2 and i+1 or i) - 1) * 73, 197, { index = SL:GetMetaValue("ITEM_INDEX_BY_NAME",v[1]),count = v[2], look = true, bgVisible = false })
                if i  == 2 then
                    GUI:ItemShow_Create(node, "cost"..3, 382 + (i) * 73, 197, { index = SL:GetMetaValue("ITEM_INDEX_BY_NAME",npc._config.now), look = true, bgVisible = false })
                end 
            end
