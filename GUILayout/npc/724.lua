...
local SPEC = {
    folder = "守城士兵甲",
    reward = {x = 182, y = 88 + 60},
    buttons = {
        [1] = {x = 502, y = 82 + 50, takeSkin = "res/custom/all_story_mission/2/btn_take.png", giveSkin = "res/custom/all_story_mission/2/btn_give.png"},
    },
}
...
local function _button_skin(actionSpec, label, state)
    if actionSpec then
        local stateSkin
        if _toint(state) <= 0 then
            stateSkin = actionSpec.takeSkin
        elseif _toint(state) < 2 then
            stateSkin = actionSpec.giveSkin
        end
        if stateSkin then
            return _asset(stateSkin) or _skin(stateSkin)
        end
        if actionSpec.skin then
    ...
    for idx, action in ipairs(list) do
        ...
        if not actionSpec.hidden then
            ...
            elseif (useDemo and actionSpec.visible ~= true) or actionSpec.transparent then
                ...
                local skin = _button_skin(actionSpec, action.label, state)
