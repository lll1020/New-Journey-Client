...
local BTN_UNLOCK_MAP = "res/custom/all_story_mission/3/btn_key.png"
local BTN_TAKE_TASK = "res/custom/all_story_mission/2/btn_take.png"
local SPEC = {
    folder = "雪域特使",
    cost = false,
    progressPos = {x = 502, y = 220},
    buttons = {
        [1] = {x = 502, y = 82},
    },
}
...
local function _render_progress(node, key, data, cfg)
    ...
    _create_text(node, "progress", pos.x or pos[1], pos.y or pos[2], 24, "#6FD3FF", string.format("当前杀怪数量：%d/%d", cur, need), {font = "fonts/502.ttf", outlineColor = "#C92A2A", outlineSize = 2})
end
    ...
    for idx, action in ipairs(list) do
        ...
        if not actionSpec.hidden then
            ...
            elseif (useDemo and actionSpec.visible ~= true) or actionSpec.transparent then
                ...
                if action.ew == 1 then
                    skin = _skin(state <= 0 and BTN_TAKE_TASK or BTN_UNLOCK_MAP) or skin
                end
