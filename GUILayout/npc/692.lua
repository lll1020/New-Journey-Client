...
npc._config = teshudata["npc_692"]

local WINDOW_OPTS = {
    background = {skin = "res/custom/all_story_mission/5/692_bg.png"},
...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function UI_updata(node)
        ...
        GUI:Image_Create(node, "fgx", 120, 145, "res/custom/all_story_mission/5/生命边界之谜/分割线-.png")
        ...
        for i, info in ipairs(REQUIRED_TASKS) do
            ...
            createText(row, "name", 15, 10, 24, "#63F7FF", taskName)
            if rewardList and #rewardList > 0 then
                local reward = ItemNumByTable_img_new(rewardList, nil, GUI:Node_Create(row, "reward", 195 + 70 + 17, 5))
