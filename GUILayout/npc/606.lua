local npc = {}
npc._config = teshudata["npc_606"]
local WINDOW_OPTS = {
    background = {skin = "res/custom/all_story_mission/2/606_bg.png"},
...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function UI_updata(node)
        ...
        if npc.data.jq_data[key] == 0 then
            ...
            NPC_UI_HELPER.tryStartMainlineUpgradeGuide(npc, Button, node, npcid, "story_606_take", {
                taskMap = {[npcid] = 31},
                keyPrefix = "mainline_story_606",
                dir = 5,
                desc = "点击领取任务",
            })
        elseif npc.data.jq_data[key] == 1 then
            local desc = GUI:Text_Create(node, "desc",100,100, 20, "#F4D179", "当前击杀：\n[红名怪] "..(npc.data.sg_data[key] or 0).."/"..(npc._config.num or 0))
            ...
            NPC_UI_HELPER.tryStartMainlineUpgradeGuide(npc, Button, node, npcid, "story_606_submit", {
                taskMap = {[npcid] = 31},
                keyPrefix = "mainline_story_606",
                dir = 5,
                desc = "点击提交任务",
