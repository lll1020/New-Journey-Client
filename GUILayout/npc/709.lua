...
local WINDOW_OPTS = {
    background = {skin = "res/custom/all_story_mission/5/709_bg.png"},
    closeButton = {x = 747, y = 380},
}
local key = "npc_709"
local btn_pos = {620, 80}
local reward_pos = {180 + 240, 80 + 70}
local cost_pos = {620, 80 + 70}
local MAIN_BTN_SKIN = "res/custom/all_story_mission/5/709_btn.png"
local MAIN_BTN_SKIN_TAKE = nil
local MAIN_BTN_SKIN_DOING = nil
local EXTRA_BTN_SKIN = {}
local ACTIONS = {1}
local ACTION_LABEL = { [1] = "执行" }
...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function UI_updata(node)
        ...
        -- 打印并显示任务坐标（道具使用坐标）。
        local use_pos = task_cfg.use_pos
        if type(use_pos) == "table" then
            local use_map = tostring(use_pos[1] or "")
            local use_x = tonumber(use_pos[2] or 0) or 0
            local use_y = tonumber(use_pos[3] or 0) or 0
