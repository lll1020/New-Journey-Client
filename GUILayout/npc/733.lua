...
local SPEC = {
    ...
    submitSlots = {
        {x = 148, y = 126, itemX = 95, itemY = 166},
        {x = 314, y = 126, itemX = 261, itemY = 166},
        {x = 480, y = 126, itemX = 427, itemY = 166},
        {x = 646, y = 126, itemX = 593, itemY = 166},
    },
    buttons = {
        [1] = {hidden = true},
    },
...
local function _create_text(parent, name, x, y, size, color, text, opts)
    ...
    GUI:Text_setFontName(label, opts.font or "fonts/font4.ttf")
...
local function _pick_items(cfg, source)
    ...
    if source == "reveal_cost" then
        return _normalize_items(task.reveal_cost)
    end
...
local function _resolve_bg()
    local main = _asset((SPEC.folder or "") .. ".png")
    if main then
        return main, false
    end
...
local function _render_item_group(node, cfg, group)
    ...
    local items = _pick_items(cfg, source)
    if not items then
        return
    end
...
local function _render_items(node, cfg)
...
local function _render_progress(node, key, data, cfg)
    local task = cfg.task_cfg or {}
...
local function _button_skin(actionSpec, label)
...
local function _render_submit_slots(node, key, data, cfg)
    local task = (cfg and cfg.task_cfg) or {}
    local submit = _normalize_items(task.submit or cfg.cost) or {}
    local submitState = ((data.T_dljq or {})[key .. "_submit"])
    submitState = type(submitState) == "table" and submitState or {}
    for idx, slot in ipairs(SPEC.submitSlots or {}) do
        local item = submit[idx]
        if item then
            local itemNode = GUI:Node_Create(node, "submit_item_" .. tostring(idx), 0, 0)
            local itemWidget = checkItemNumByTable_img_kuang({item}, nil, itemNode)
            if itemWidget then
                GUI:setPosition(itemWidget, slot.itemX or slot.x, slot.itemY or 78)
            end
        end
        ...
        if done then
            ...
            GUI:addOnClickEvent(btn, function()
                SL:SendLuaNetMsg(100, NPC_ID, 1, idx, "")
            end)
        end
    end
end
local function _render_buttons(node, key, data, cfg, useDemo)
    ...
    for idx, action in ipairs(list) do
        ...
        if not actionSpec.hidden then
            ...
            elseif (useDemo and actionSpec.visible ~= true) or actionSpec.transparent then
                ...
                local skin = _button_skin(actionSpec, action.label)
...
local function _ensure_window()
    local bgSkin, useDemo = _resolve_bg()
    ...
    if _valid(npc.node) and (not _valid(npc.staticNode) or not _valid(npc.refreshNode)) then
        GUI:removeAllChildren(npc.node)
        npc.staticNode = GUI:Node_Create(npc.node, "staticNode", 0, 0)
        npc.refreshNode = GUI:Node_Create(npc.node, "refreshNode", 0, 0)
        _render_images(npc.staticNode)
    end
    return npc.node
end

local function _render()
    if not _valid(npc.node) or not _valid(npc.refreshNode) then
        _ensure_window()
    end
    local node = npc.refreshNode
    ...
    _render_items(node, curCfg)
    _render_submit_slots(node, key, npc.data, curCfg)
    _render_progress(node, key, npc.data, curCfg)
    _render_buttons(node, key, npc.data, curCfg, npc._useDemo)
end
