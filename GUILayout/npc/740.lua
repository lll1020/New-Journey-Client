...
local SPEC = {
    folder = "上古寒冰剑",
    itemGroups = {
        {name = "weapon", source = "craft_reward", kind = "reward", x = 590 - 5, y = 268 + 38},
    },
    singleCosts = {
        {index = 1, x = 240 + 210, y = 174 + 41},
        {index = 2, x = 370 + 154, y = 174 + 41},
        {index = 4, x = 495 + 104, y = 174 + 41},
        {index = 3, x = 620 + 52, y = 174 + 41},
    },
    completeAction = 2,
    buttons = {
        [1] = {hidden = true},
        [2] = {x = 410, y = 82, skin = "立即合成.png"},
    },
}
...
local function _render_single_costs(node, cfg)
    if type(SPEC.singleCosts) ~= "table" then
        return
    end
    local task = (cfg and cfg.task_cfg) or {}
    local costs = _normalize_items(task.craft_cost or task.submit or cfg.cost)
    if not costs then
        return
    end
    for _, pos in ipairs(SPEC.singleCosts) do
        local item = costs[_toint(pos.index, 0)]
        if item then
            local root = GUI:Node_Create(node, "single_cost_" .. tostring(pos.index), 0, 0)
            local widget = checkItemNumByTable_img_kuang({item}, nil, root)
            if widget then
                GUI:setPosition(widget, pos.x or 0, pos.y or 0)
            end
        end
    end
end
...
local function _render_buttons(node, key, data, cfg, useDemo)
    ...
    for idx, action in ipairs(list) do
        ...
        if not actionSpec.hidden then
            ...
            if state >= 2 and action.ew == (SPEC.completeAction or 1) then
...
local function _render()
    ...
    _render_single_costs(node, curCfg)
