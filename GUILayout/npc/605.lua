...
npc._config = teshudata["npc_605"]



local WINDOW_OPTS = {
    background = {skin = "res/custom/all_story_mission/2/605_bg.png"},
...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function UI_updata(node)
        ...
        GUI:setPosition(jl, 390 - 285, 135)
        ...
        if npc.data.jq_data[key] == 0 then
            ...
            NPC_UI_HELPER.tryStartMainlineUpgradeGuide(npc, Button, node, npcid, "story_605_take", {
                taskMap = {[npcid] = 27},
                keyPrefix = "mainline_story_605",
                dir = 5,
                desc = "点击领取任务",
            })
        elseif npc.data.jq_data[key] == 1 then
            local desc = GUI:Text_Create(node, "desc",300,220, 20, "#F4D179", "当前击杀："..(npc.data.sg_data[key] or 0))
            ...
            NPC_UI_HELPER.tryStartMainlineUpgradeGuide(npc, Button, node, npcid, "story_605_submit", {
                taskMap = {[npcid] = 27},
                keyPrefix = "mainline_story_605",
                dir = 5,
                desc = "点击提交任务",
