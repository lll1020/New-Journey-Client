...
local SPEC = {
    ...
    buttons = {
        [2] = {x = 425, y = 82, skin = "揭示弱点.png"},
        [3] = {x = 650, y = 82, skin = "进入副本.png"},
    },
}
...
local function _render_weak_text(node, key, data)
    local story = (data and data.T_dljq) or {}
    local text = _toint(story[key .. "_weak"]) == 1 and "惧怕灵兽火焰" or "强大且神秘"
    local color = _toint(story[key .. "_weak"]) == 1 and "#35FF58" or "#FF4040"
    local label = GUI:Text_Create(node, "weak_text", 438 - 106, 198, 22, color, text)
    GUI:setAnchorPoint(label, 0.5, 0.5)
    GUI:Text_setFontName(label, "fonts/502.ttf")
    GUI:Text_enableOutline(label, "#1A0000", 2)
end
local function _render_title_reward(node, cfg)
    local titleName = cfg and cfg.ch or ((cfg and cfg.task_cfg) and cfg.task_cfg.ch)
    if not titleName or titleName == "" then
        return
    end
    local itemName = tostring(titleName) .. "[称号]"
    local root = GUI:Node_Create(node, "title_reward", 0, 0)
    local itemNode = ItemNumByTable_img_new({{itemName, 1}}, nil, root)
    if itemNode then
        GUI:setPosition(itemNode, 170, 80)
    end
end
...
local function _render()
    ...
    _render_weak_text(node, key, npc.data)
    _render_title_reward(node, curCfg)
