...
npc._config = teshudata["npc_688"]

local WINDOW_OPTS = {
    background = {skin = "res/custom/all_story_mission/5/688_bg.png"},
    closeButton = {x = 747, y = 380},
}
local key = "npc_688"
local btn_pos = {620 - 230, 55}
local reward_pos = {360, 120}
local cost_pos = {520, 200}
local MAIN_BTN_SKIN = "res/custom/all_story_mission/5/688_btn.png"
local MAIN_BTN_SKIN_TAKE = "res/custom/all_story_mission/2/btn_take.png"
local MAIN_BTN_SKIN_DOING = nil
local EXTRA_BTN_DIR = "res/custom/all_story_mission/5/688/list"
local ACTIONS = {1, 2, 3, 4}
local ACTION_LABEL = { [1] = "解锁", [2] = "倚天江湖", [3] = "三国乱世", [4] = "水浒再临" }
...
function npc.main(npcid, p2, p3, msgData)
    ...
    local function createActionButtons(node, state, progress, unlockMaps, dljqData)
        ...
        local function createExtraButton(nodeName, x, y, ew)
            ...
            if not isUnlocked then
                GUI:setAnchorPoint(GUI:Image_Create(btn, "lock", 196/2, 300/2, "res/custom/all_story_mission/5/688/lock.png")
                , 0.5, 0.5)
            end
    ...
    local function UI_updata(node)
        ...
        GUI:Image_Create(node, "wz", 70, 0, "res/custom/all_story_mission/5/688/wz.png")
        -- if cost_cfg then
        ...
        if kill_need > 0 then
            ...
            local t = GUI:Text_Create(node, "progress", 500, 40, 20, "#808080", string.format("击杀 %d/%d(可累计)", kill_cur, need_total))
