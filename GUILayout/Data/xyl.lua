...
-- 备注：本命灵根是否升级过一次
local function _xyl_has_main_linggen_upgraded()
    local data = _xyl_get_json("T41")
...
local function _xyl_has_lingshou_hatched()
    local data = _xyl_get_json("T50")
    if tonumber(data.baby_choice or 0) and tonumber(data.baby_choice or 0) > 0 then
        return true
    end
...
local function _xyl_has_xuanyuan_material()
...
local function _xyl_has_lingshou_star(star)
    ...
    local data = _xyl_get_json("T50") or {}
...
local function _xyl_check_task(name)
    ...
    local special = {
        ...
        ["灵兽全二星"] = function() return _xyl_has_lingshou_star(2) end,
        ["灵兽全三星"] = function() return _xyl_has_lingshou_star(3) end,
...
local function _xyl_is_extra_jqd_chapter(cfg)
    ...
    return _XYL_EXTRA_JQD_CHAPTERS[tostring(cfg.name or "")] == true
end
local function _xyl_get_extra_completed_jqd_before(taskData, l, zj)
    l = tonumber(l) or 0
    zj = tonumber(zj) or 0
    local total = 0
    for lIdx, lCfg in ipairs(taskData or {}) do
        if type(lCfg) == "table" then
            for zjIdx, cfg in ipairs(lCfg) do
                if lIdx < l or (lIdx == l and zjIdx < zj) then
                    if _xyl_is_extra_jqd_chapter(cfg) then
                        total = total + _xyl_get_completed_chapter_reward_jqd(taskData, lIdx, zjIdx)
                    end
                end
            end
        end
...
local function _xyl_adjust_unlock_jqd(taskData, l, zj, curJqd)
    ...
    local extraJqd = _xyl_get_extra_completed_jqd_before(taskData, l, zj)
    if extraJqd <= 0 then
        return nowJqd
    end
    nowJqd = nowJqd - extraJqd
    if nowJqd < 0 then
        nowJqd = 0
    end
    return nowJqd
...
local npc_xyl = {
        ...
        },
        {
            jq = {
                ...
                },
                {
                    ...
                    need_receive = false,
                    yd = { 1, "不周山", 675, 75, 77 },
                    desc = "将逐日弓穿戴到背包神器槽位后，不断消耗箭矢为同一把弓追加神秘词条。\n<font color='#F4D179'>目标：</font>同一把神器槽逐日弓累计拥有9条词条\n当前箭矢：%s",
                },
                {
                    "共公怒触不周山",
                    tk = "npc_676",
    ...
    },
    {
        ...
        {
            jq = {
                {"盛世重游", tk = "npc_733", id = 999, jl = {{"剧情点", 1}}, fwdjy = nil, khdjy = _xyl_khdjy, need_receive = false, yd = {1, "六大陆主城", 733, 82, 113}, desc = "重游古城，集齐四地信物再来交付。\n<font color='#F4D179'>目标：</font>提交四件城市任务道具\n<font color='#F4D179'>进度：</font>%s"},
                {"万国之首", tk = "npc_734", id = 999, jl = {{"剧情点", 1}}, fwdjy = nil, khdjy = _xyl_khdjy, need_receive = false, yd = {1, "长安西市", 734, 209, 126}, desc = "商契残页散落西市。\n<font color='#F4D179'>目标：</font>收集88张商契残页\n<font color='#F4D179'>进度：</font>%s"},
                {"洛水杜康", tk = "npc_735", id = 999, jl = {{"剧情点", 1}}, fwdjy = nil, khdjy = _xyl_khdjy, need_receive = false, yd = {1, "洛阳天街", 735, 85, 68}, desc = "酒意与泉眼相合，方得杜康。\n<font color='#F4D179'>目标：</font>备齐材料酿杜康酒\n<font color='#F4D179'>进度：</font>%s"},
                {"大宋的菜肴", tk = "npc_736", id = 999, jl = {{"剧情点", 1}}, fwdjy = nil, khdjy = _xyl_khdjy, need_receive = false, yd = {1, "汴京御街", 736, 91, 53}, desc = "食盒空置，菜肴才是答案。\n<font color='#F4D179'>目标：</font>提交食盒与菜肴\n<font color='#F4D179'>进度：</font>%s"},
