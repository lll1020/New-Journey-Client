...
function SettingBasic.initGroup4()
    ...
    local Panel_Voice = GUI:Layout_Create(ListView, "Panel_Voice_ew", 0, 0, 175, 100, false)
    -- 描述
    local Text_desc = GUI:Text_Create(Panel_Voice, "Text_desc", 13, 71, 12, "#ffffff", "设置分辨率")
    GUI:setAnchorPoint(Text_desc, 0, 0.5)
    local Image_2 = GUI:Image_Create(Panel_Voice, "Image_2", 10, 25, "res/wy/public/00.png")
    GUI:setTouchEnabled(Image_2, true)
    local Text_name = GUI:Text_Create(Image_2, "Text_name", 5, 5, 16, "#109c18", cogin.w.."×"..cogin.h)
    GUI:addOnClickEvent(Image_2, function()
        ...
        SL:OpenSelectListUI({"1920×1080","1600×1024","1600×900","1440×900","1366×768","1280×800","1280×768","1152×864","1024×768","800×600"}, position, size.width, size.heigth, function(idx)
            if idx ~= 0 then
                GUI:Text_setString(Text_name, resolutions[idx][1].."×"..resolutions[idx][2])
                SL:SetMetaValue("WIN_DEVICE_SIZE", resolutions[idx][1], resolutions[idx][2])
...
function SettingBasic.CreateVoiceProgressCell(parent, data)
    local Panel_Voice = GUI:Layout_Create(parent, "Panel_Voice_" .. data.id, 0, 0, 200, 100, false)
    --设置数据
    ...
    GUI:setContentSize(Slider_progress, 160, 11)
