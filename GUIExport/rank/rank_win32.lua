...
function ui.init(parent)
	...
	-- Create Panel_1
	local Panel_1 = GUI:Layout_Create(Scene, "Panel_1", 568.00, 300.00, 846.00, 606.00, false)
	...
	-- Create Panel_bg
	local Panel_bg = GUI:Layout_Create(Panel_1, "Panel_bg", 0.00, 0.00, 846.00, 606.00, false)
	GUI:setChineseName(Panel_bg, "排行榜_背景组合")
	GUI:setTouchEnabled(Panel_bg, false)
	GUI:setTag(Panel_bg, 31)

	-- Create Image_bg
	local Image_bg = GUI:Image_Create(Panel_bg, "Image_bg", 0.00, 0.00, "res/wy/public/phb.png")
	GUI:Image_setScale9Slice(Image_bg, 282, 283, 202, 202)
	GUI:setContentSize(Image_bg, 847, 606)
	...
	-- Create Panel_type
	local Panel_type = GUI:Layout_Create(Panel_1, "Panel_type", 107.00, 441.00, 185.00, 45.00, false)
	...
	local Panel_btnList = GUI:Layout_Create(Panel_1, "Panel_btnList", -19.00, 110.00, 60.00, 310.00, false)
    ...
    local xulie1 = GUI:Frames_Create(Panel_1, "xulie1", 425, 483, "res/wy/eff/ui/0_", ".png", 1, 15, {count=15,speed=50,loop=-1})
    GUI:setAnchorPoint(xulie1, 0.50, 0.50)

    local xulie2 = GUI:Frames_Create(Panel_1, "xulie2", 422, 146, "res/wy/eff/ui/1_", ".png", 1, 15, {count=15,speed=50,loop=-1})
    GUI:setAnchorPoint(xulie2, 0.50, 0.50)

	-- Create ListView_list
	local ListView_list = GUI:ListView_Create(Panel_1, "ListView_list", 112.00, 73.00, 370.00, 360.00, 1)
	...
	-- Create Node_model
	local Node_model = GUI:Node_Create(Panel_1, "Node_model", 595.00, 231.00)
	GUI:setChineseName(Node_model, "排行榜_模型_节点")
	GUI:setAnchorPoint(Node_model, 0.50, 0.50)
	GUI:setTag(Node_model, 47)

	-- Create Button_looks
	local Button_looks = GUI:Button_Create(Panel_1, "Button_looks", 592.00, 60.00, "res/public_win32/1900000679.png")
	...
	-- Create Button_close
	local Button_close = GUI:Button_Create(Panel_1, "Button_close", 776.00, 470.00, "res/wy/public/999.png")
	GUI:Button_setScale9Slice(Button_close, 19, 20, 19, 18)
	GUI:setContentSize(Button_close, 58, 56)
	...
	-- Create TouchSize
	local TouchSize = GUI:Layout_Create(Button_close, "TouchSize", -6.00, 0.00, 70.00, 70.00, false)
	...
	-- Create Panel_myInfo
	local Panel_myInfo = GUI:Layout_Create(Panel_1, "Panel_myInfo", 112.00, 36.00, 370.00, 26.00, false)
